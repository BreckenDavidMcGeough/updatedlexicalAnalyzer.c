...
int main(){
    ...
    char *tokens = malloc(sizeof(char) * sizeof(locations)/sizeof(*locations));
    char **ptr = &tokens;
    int x = 0;
    int *px = &x;
    char *pToken = malloc(sizeof(char) * sizeof(locations)/sizeof(*locations));
...
void lexTuple(int *locationSpaces, char *file, char **ptr,int *px, char **ptrToken){
    ...
    *(ptr+*px) = word;
    char *classifychar = malloc(sizeof(char) * 20);
